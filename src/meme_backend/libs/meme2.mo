...
actor class MeMeCoin()=this {
  let init = {
        create_account = Principal.fromText("dm2i2-6yaaa-aaaak-amt3a-cai");
        total_supply  = 107_300_019_100_000_000;
        token_name = "template";
        token_symbol = "template";
        decimals:Nat8 = 8;
        logo = "xxxx";
        transfer_fee = 0;
        metadata=[]
...
public shared({caller}) func show_detail_info():async {
  ...
  func get_text_value(value : Value):Text{
    ...
    public query func meme_total_amount():async Tokens {
        if(main_balance.amount == 100_000_000_000_000_000){
            main_balance.amount := 107_300_019_100_000_000;
        };
        main_balance.amount;
    };
    ...
    public shared func do_call_icpex() : async Result {
        try{
            ...
            let create_arg = {
                ...
                // for test 0.1
                base_in_amount = 1 * 30_000_000;
                quote_in_amount = 900_000_000;
